...
<% for attribute in attributes -%>
      And I should see "<<%= attribute.name %>>"
<% end -%>
      Examples:
...
<% for attribute in attributes -%>
        | <%= singular_table_name %>_<%= attribute.name %> | <%= attribute.default %> |
<% end -%>
      And I check "<%= singular_table_name %>_assigned_answer_answer_id_1"
      And I check "<%= singular_table_name %>_assigned_answer_answer_id_4"
      And I press "Dodaj"
    And I should see "Zabieg kosmetyczny został pomyślnie utworzony."
      And <%= singular_table_name %> should exist with <%= attributes.map {|attribute| "#{attribute.name}: \"#{attribute.default}\"" }.join(', ') -%>
      And assigned answer should exist with <%= attributes.first.name %>: "<%= attributes.first.default %>"
      And assigned answer should exist with <%= attributes.first.name %>: "<%= attributes.first.default %>"
...
<% end -%>
    ...
    Then I should see "<%= human_name %> został pomyślnie uaktualniony."
  ...
  Scenario: Successful deletion of <%= singular_table_name %>
    ...
    Then I should see "Zabieg kosmetyczny został pomyślnie usunięty."
